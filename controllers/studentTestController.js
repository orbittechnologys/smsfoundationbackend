...
export const getTestsForStudent = asyncHandler(async (req, res) => {
  try {
    ...
    if (!student) {
      return res
        .status(400)
        .json({ success: false, msg: "No such student found " + studentId });
    }

    const studentTests = await studentTest
...
export const getTestReport = asyncHandler(async (req, res) => {
  try {
    ...
    if (!testReport || testReport.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No test reports found." });
    }
    ...
    return res
      .status(200)
      .json({ success: true, testReport: modifiedTestReport });
  } catch (error) {
...
export const getTestResults = asyncHandler(async (req, res) => {
  try {
    ...
    const testResults = await studentTest
        ...
        $addFields: {
          percentage: {
            $round: [
              ...
              2,
            ], // Specify rounding to 2 decimal places
          },
        },
      },
    ]);
...
export const fetchLatestStudentTests = async (req, res) => {
  try {
    ...
    const latestTests = await studentTest
      .find({ student: studentId })
      .populate("test")
...
export const getTestResult = asyncHandler(async (req, res) => {
  try {
    ...
    console.log(
      `Fetching student test document for student ID: ${studentId} and test ID: ${testId}`
    );
...
export const getTestReportForStudent = asyncHandler(async (req, res) => {
  try {
    const studentId = req.params.studentId;
    ...
    const testReports = await studentTest
      .find({ student: studentId })
      .populate({
        path: "test",
        select: "name totalMarks", // Get test details
      })
      .lean();
    ...
    // Format test report details
    const formattedTestReport = testReports.map((report) => ({
      testId: report.test?._id,
      testName: report.test?.name,
      totalMarks: report.test?.totalMarks || 0,
      obtainedMarks: report.marks,
      percentage: report.test?.totalMarks
        ? ((report.marks / report.test.totalMarks) * 100).toFixed(2)
        : "0.00",
    }));
    ...
    return res.status(200).json({
      ...
      studentDetails: {
        ...
        school: {
          id: studentDoc.school?._id,
          name: studentDoc.school?.name || "N/A",
        },
      },
      testReport: formattedTestReport,
    });
  } catch (error) {
...
export const getLearningReportForStudent = asyncHandler(async (req, res) => {
  try {
    const studentId = req.params.studentId;
